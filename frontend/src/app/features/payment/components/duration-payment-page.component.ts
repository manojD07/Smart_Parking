              ...
              <div class="row" *ngIf="paymentSession">
                ...
                <div class="col-md-6">
                  ...
                  <div class="selected-time-slot">
                    <div class="time-slot-item" *ngIf="paymentSession.selectedChunks?.length">
                      <i class="fas fa-clock me-1"></i>
                      {{ formatChunkTime(paymentSession.selectedChunks[0].start_time) }} - 
                      {{ formatChunkTime(paymentSession.selectedChunks[paymentSession.selectedChunks.length - 1].end_time) }}
                    </div>
                    <div class="time-slot-item" *ngIf="!paymentSession.selectedChunks?.length">
                      <i class="fas fa-clock me-1"></i>
                ...
                <div class="alert alert-warning mb-3">
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="simulateFailure">
                      <strong>Simulate payment failure (for testing)</strong>
                    </label>
                  </div>
                  <small class="text-muted">Demo Mode: Check this box to test payment failure scenarios</small>
                </div>
...
export class DurationPaymentPageComponent implements OnInit, OnDestroy {
  ...
  private initializePaymentSession() {
    this.route.queryParams.pipe(takeUntil(this.destroy$)).subscribe(params => {
      if (params['sessionId']) {
        ...
        console.warn('⚠️ No session ID found, creating mock session for testing');
        ...
        this.paymentSession = {
          ...
          totalAmount: 25.50,
          expiresAt: new Date(now.getTime() + 10 * 60 * 1000).toISOString() // 10 minutes from now
        };
        console.log('✅ Created mock payment session:', this.paymentSession);
      }
    });
  }
  ...
  calculateDuration(): number {
    if (!this.paymentSession?.selectedChunks?.length) {
      // If no chunks, create mock chunks based on session data
      if (this.paymentSession) {
        console.log('🔍 No selectedChunks found, creating mock chunks');
        // Create 2 chunks (1 hour) as default
        const now = new Date();
        const mockChunks = [
          {
            start_time: now.toISOString(),
        ...
        this.paymentSession.selectedChunks = mockChunks;
        console.log('✅ Created mock chunks:', mockChunks);
        return 1; // 1 hour
      }
      return 0;
    }
  ...
  async processPayment() {
    ...
    try {
      ...
      // Check if user wants to simulate failure
      const simulateFailure = this.paymentForm.get('simulateFailure')?.value || false;
      
      if (simulateFailure) {
        throw new Error(`${this.selectedPaymentMethod} payment failed. Simulated failure for testing.`);
      }
      ...
      if (isMockSession) {
        ...
        this.router.navigate(['/bookings'], {
          queryParams: {
            ...
            message: `Payment successful via ${this.getPaymentMethodName()}! Your parking slot has been booked. (Demo Mode)`
          }
        });
      } else {
